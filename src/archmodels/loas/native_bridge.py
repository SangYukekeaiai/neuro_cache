...
def _unpack_output(data: bytes, num_tiles: int, sample_indices: Sequence[int]):
    """Parses the tick-grouped wire format loasgen's main.cpp now writes
    ...
    expects -- no further adaptation needed for loas."""
    off = 0
    n = len(data)
    ...
    assert off == n, f"loasgen output: {off} bytes consumed, {n} in file (framing bug)"
...
def reconstruct_samples_native(
    ...
    or just dram_i (single-node, noc_i=core_id=0 for every tile) -- one
    subprocess call handles the whole flat list either way, since
    reconstruct_sample/TileSpec already take arbitrary per-tile offsets/
    bounds generically (see log/2026-08-02-multinode-core-driven-weight-trace-plan.md).
    ...
    if not _BINARY.exists():
        raise FileNotFoundError(
            f"loasgen binary not found at {_BINARY} -- build it first: "
