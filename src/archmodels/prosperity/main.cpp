// prosperitygen -- CLI wrapper around ProsperityGen.h's reconstruction port.
//
// Usage:
//   prosperitygen <trace.bin> <task.bin> <out.bin>
//
// task.bin (all int32, native-endian):
//   T, B_full, Cin_full, Hin_full, Win_full
//   num_tiles
//   num_samples
//   per tile (num_tiles times): dram_i, ho_off, ho_n, wo_off, wo_n,
//     kh_n, kw_n, cin, t, cout_off, cout_n  (11 int32)
...
// Prosperity's own tick assignment (ProsperityGen.h) is strictly
...
int32_t read_i32(std::ifstream& fh) {
    ...
    if (!fh) {
        std::cerr << "prosperitygen: unexpected EOF reading task file\n";
        std::exit(2);
    }
    return v;
}
...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::cerr << "usage: prosperitygen <trace.bin> <task.bin> <out.bin>\n";
        return 2;
    }
    ...
    if (!task_fh) {
        std::cerr << "prosperitygen: cannot open task file " << task_path << "\n";
        return 2;
    }
    ...
    for (auto& tile : tiles) {
        ...
        tile.cin = read_i32(task_fh);
        tile.t = read_i32(task_fh);
    ...
    std::vector<uint8_t> trace(trace_elems);
    {
        ...
        if (!trace_fh) {
            std::cerr << "prosperitygen: cannot open trace file " << trace_path << "\n";
            return 2;
        }
        trace_fh.read(reinterpret_cast<char*>(trace.data()), trace_elems);
        if (!trace_fh) {
            std::cerr << "prosperitygen: trace file shorter than expected shape\n";
            return 2;
        }
    }
    ...
    if (out_path == "-") {
        ...
        if (!out_file) {
            std::cerr << "prosperitygen: cannot open output file " << out_path << "\n";
            return 2;
        }
        out = &out_file;
    }
