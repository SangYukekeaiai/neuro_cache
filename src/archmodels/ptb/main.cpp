// ptbgen -- CLI wrapper around PTBGen.h's reconstruction port.
//
// Usage:
//   ptbgen <trace.bin> <task.bin> <out.bin>
//
...
// PTB's own tick assignment (PTBGen.h) is strictly sequential, so every
// tick group here has exactly one address.
...
int32_t read_i32(std::ifstream& fh) {
    ...
    if (!fh) {
        std::cerr << "ptbgen: unexpected EOF reading task file\n";
        std::exit(2);
    }
    return v;
}
...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::cerr << "usage: ptbgen <trace.bin> <task.bin> <out.bin>\n";
        return 2;
    }
    ...
    if (!task_fh) {
        std::cerr << "ptbgen: cannot open task file " << task_path << "\n";
        return 2;
    }
    ...
    std::vector<uint8_t> trace(trace_elems);
    {
        ...
        if (!trace_fh) {
            std::cerr << "ptbgen: cannot open trace file " << trace_path << "\n";
            return 2;
        }
        trace_fh.read(reinterpret_cast<char*>(trace.data()), trace_elems);
        if (!trace_fh) {
            std::cerr << "ptbgen: trace file shorter than expected shape\n";
            return 2;
        }
    }
