// spinalflowgen -- CLI wrapper around SpinalFlowGen.h's reconstruction port.
//
// Usage:
//   spinalflowgen <trace.bin> <task.bin> <out.bin>
//
...
int32_t read_i32(std::ifstream& fh) {
    ...
    if (!fh) {
        std::cerr << "spinalflowgen: unexpected EOF reading task file\n";
        std::exit(2);
    }
    return v;
}
...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::cerr << "usage: spinalflowgen <trace.bin> <task.bin> <out.bin>\n";
        return 2;
    }
    ...
    if (!task_fh) {
        std::cerr << "spinalflowgen: cannot open task file " << task_path << "\n";
        return 2;
    }
    ...
    std::vector<uint8_t> trace(trace_elems);
    {
        ...
        if (!trace_fh) {
            std::cerr << "spinalflowgen: cannot open trace file " << trace_path << "\n";
            return 2;
        }
        trace_fh.read(reinterpret_cast<char*>(trace.data()), trace_elems);
        if (!trace_fh) {
            std::cerr << "spinalflowgen: trace file shorter than expected shape\n";
            return 2;
        }
    }
    ...
    if (!out_fh) {
        std::cerr << "spinalflowgen: cannot open output file " << out_path << "\n";
        return 2;
    }
