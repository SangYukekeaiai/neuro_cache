// gustavgen -- CLI wrapper around GustavGen.h's reconstruction port.
//
// Usage:
//   gustavgen <trace.bin> <task.bin> <out.bin>
//
// task.bin (all int32, native-endian -- writer and reader run on the
// same machine, no cross-platform concern):
//   T, B_full, Cin_full, Hin_full, Win_full
//   num_tiles
//   num_samples
//   per tile (num_tiles times): dram_i, t_offset, ho_off, ho_n, wo_off,
//     wo_n, kh_n, kw_n, cin_off, cin_n, cout_off, cout_n  (12 int32)
//   per sample (num_samples times): sample_idx
//
// trace.bin: flat uint8, row-major [T, B_full, Cin_full, Hin_full,
// Win_full] -- the same layout numpy's .npy stores its array body in,
// so the Python side just writes trace.tobytes() after np.load, no
// reshaping.
//
...
int32_t read_i32(std::ifstream& fh) {
    ...
    if (!fh) {
        std::cerr << "gustavgen: unexpected EOF reading task file\n";
        std::exit(2);
    }
    return v;
}
...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::cerr << "usage: gustavgen <trace.bin> <task.bin> <out.bin>\n";
        return 2;
    }
    ...
    if (!task_fh) {
        std::cerr << "gustavgen: cannot open task file " << task_path << "\n";
        return 2;
    }
    ...
    for (auto& tile : tiles) {
        ...
        tile.t_offset = read_i32(task_fh);
        tile.ho_off = read_i32(task_fh);
        tile.ho_n = read_i32(task_fh);
        tile.wo_off = read_i32(task_fh);
        tile.wo_n = read_i32(task_fh);
    ...
    std::vector<uint8_t> trace(trace_elems);
    {
        ...
        if (!trace_fh) {
            std::cerr << "gustavgen: cannot open trace file " << trace_path << "\n";
            return 2;
        }
        trace_fh.read(reinterpret_cast<char*>(trace.data()), trace_elems);
        if (!trace_fh) {
            std::cerr << "gustavgen: trace file shorter than expected shape\n";
            return 2;
        }
    }
    ...
    if (!out_fh) {
        std::cerr << "gustavgen: cannot open output file " << out_path << "\n";
        return 2;
    }
