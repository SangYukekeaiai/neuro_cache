...
def _unpack_output(data: bytes, num_tiles: int, sample_indices: Sequence[int]):
    """Yields (tile_idx, sample_idx, mac_cycles, addresses, tick_ids) in
    the exact order main.cpp wrote them: tile-major, sample-minor."""
    off = 0
    n = len(data)
...
def reconstruct_samples_native(
    ...
    # per_sample_tiles[i][tile_idx] -- mirrors
    # reconstruct_samples_for_schedule's own per_sample_tiles layout.
    ...
    for tile_idx, local_idx, mac_cycles, addresses, ticks in _unpack_output(out_data, len(tiles), local_indices):
        i = local_idx  # local index IS the position, by construction
