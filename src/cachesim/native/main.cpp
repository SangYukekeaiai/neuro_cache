...
// profiling/0726/native/cache_sweep.cpp's specialized all-288-at-once
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        for (uint32_t ei = 0; ei < n_events; ++ei) {
            int32_t v0 = raw[ei * 5 + 0], v1 = raw[ei * 5 + 1], v2 = raw[ei * 5 + 2], v4 = raw[ei * 5 + 4];
        ...
        for (uint32_t ei = 0; ei < n_events; ++ei) {
            ...
            for_each_element(v, order, [&](const Element &e) {
                ...
                bool hit = cache.access(tag_sum(t), packer.pack(t));
                hits += hit ? 1 : 0;
                ++total;
            });
        }
