#!/usr/bin/env python3
"""L1 16KB vs 8KB, prefetcher OFF, all 31 layers, against the nocsim makespan.

One grid, two arms. Everything except `l1_size_bytes` is the 0903_l1_pf_fix BASE,
so the `off_16k` arm must reproduce that stage's `off` row -- it is the control,
not a second result.
...
GRID = [dict(BASE, l1_size_bytes=16384), dict(BASE, l1_size_bytes=8192)]
ARMS = ["off_16k", "off_8k"]
...
def one(args):
    ...
    r = subprocess.run(
        [str(BIN), "--config-grid", str(grid_path),
         ...
         "--arm", "cache", "--tier", "l1cap",
         "--run-id", f"l1cap-{tag}-s{s}", "--git-commit", commit, "--header"],
...
def aggregate(rows, avail):
    ...
    base = {t: statistics.mean(float(r["total_cycles"]) for r in rs)
            for (t, a), rs in g.items() if a == "off_16k"}
    ...
    for tag, _ in tags():
        for arm in ARMS:
            ...
            row = {"tag": tag, "layer": rs[0]["layer"], "workload": rs[0]["workload"],
                   "arm": arm, "samples": len(rs),
                   "cycles": round(mc, 1),
                   "cycles_spread": round((max(cyc) - min(cyc)) / mc, 4),
                   "speedup_vs_16k": round(base[tag] / mc, 4),
    ...
    for arm in ARMS:
        ...
        a = {"arm": arm, "layers": len(rs),
             ...
             "geomean_speedup_vs_16k": round(geomean([r["speedup_vs_16k"] for r in rs]), 4),
...
def main() -> int:
    ...
    write_csv(OUT / "l1cap_rows.csv", rows)
    by_layer, agg = aggregate(rows, avail)
    write_csv(OUT / "l1cap_by_layer.csv", by_layer)
    write_csv(OUT / "l1cap_aggregate.csv", agg)
